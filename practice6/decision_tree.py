...
def decision_tree(X, Y, scale, level=0):
    ...
    for i in range(n):
        ...
        if scale[i] == CATEGORICAL:  # категориальный признак
            ...
            # 
            # Формула для вычисления InfoS(T) приведена в методических указаниях.
            ...
            # 
            # Info(Ti) - энтропия i-го подмножества. Считается с помощью функции Info, в которую передается подвектор Y,
            # соответствующий i-му подмножеству.
            #
...
def Info(set):
    ...
    # приведенной в методических указаниях. Для определения количества классов во множестве T
