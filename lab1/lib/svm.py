...
def svm_predict(model, X):
    ...
    if kernelFunctionName == 'linear_kernel':
        ...
        # возводим в степень(2) 2 столбца по номеру оси 1
...
def visualize_data(X, y, title=''):
    x1, x2 = X.T
    #ravel() возвращает сжатый до одной оси массив.
    f_y = y.ravel()
...
def visualize_boundary_linear(X, y, model, title=''):
    visualize_data(X, y, title)
    if model:
        #срез первого столбца записывая в x1
        x1 = X[:, 0]
        w = model.w
        b = model.b
        #linspace() - возвращает одномерный массив из указанного количества
        #элементов, равномерно распределенных внутри заданного интервала (start, stop, num)
